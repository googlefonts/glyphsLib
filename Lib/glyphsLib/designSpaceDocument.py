...
# FIXME: (jany) copy-pasted from https://github.com/LettError/designSpaceDocument
# TODO: move to fontTools https://github.com/LettError/designSpaceDocument/issues/28
...
class BaseDocWriter(object):
    ...
    @classmethod
    def getAxisDecriptor(cls, document):
    ...
    @classmethod
    def getInstanceDescriptor(cls, document):
        return cls.instanceDescriptorClass()

    @classmethod
    def getRuleDescriptor(cls, document):
...
class BaseDocReader(object):
    ...
    def readRules(self):
        ...
        for ruleElement in self.root.findall(".rules/rule"):
            ruleObject = self.documentObject.newRuleDescriptor()
    ...
    def readAxes(self):
        ...
        for axisElement in self.root.findall(".axes/axis"):
            axisObject = self.documentObject.newAxisDescriptor()
    ...
    def guessAxes(self):
        ...
        for axisName in maxima.keys():
            a = self.documentObject.newAxisDescriptor()
    ...
    def readSources(self):
        for sourceCount, sourceElement in enumerate(self.root.findall(".sources/source")):
            ...
            sourceObject = self.documentObject.newSourceDescriptor()
    ...
    def _readSingleInstanceElement(self, instanceElement, makeGlyphs=True, makeKerning=True, makeInfo=True):
        ...
        if filename is not None:
            ...
            instancePath = None
        instanceObject = self.documentObject.newInstanceDescriptor()
...
class DesignSpaceDocument(object):
    ...
    def newAxisDescriptor(self):
        ...
        return self.writerClass.getAxisDecriptor(self)

    def newSourceDescriptor(self):
        # Ask the writer class to make us a new sourceDescriptor
        return self.writerClass.getSourceDescriptor(self)

    def newInstanceDescriptor(self):
        # Ask the writer class to make us a new instanceDescriptor
        return self.writerClass.getInstanceDescriptor(self)

    def newRuleDescriptor(self):
        # Ask the writer class to make us a new instanceDescriptor
        return self.writerClass.getRuleDescriptor(self)
...
class InMemorySourceDescriptor(SourceDescriptor):
    """A source descriptor that has a reference to an instance of a defcon Font
    loaded in memory.
    """
    ...
    def font(self, font):
        self._font = font
        # FIXME: (jany) not sure the following is correct
        # Intent: if the given font already has a path, use it
        # else write the font next to the path specified for the descriptor
        # else write the font next to the designSpaceDocument.
        # else use a default path relative to the descriptor.
        if font.path:
            self.path = font.path
            self.filename = None
            # No need to compute the correct relative filename

        # The font's path is not updated here. Instead, a correct value
        # will be computed by the writer, depending on where the whole
        # document is going to be written.


class InMemoryDocWriter(BaseDocWriter):
    """Writes in-memory UFOs next to the designspace."""

    sourceDescriptorClass = InMemorySourceDescriptor
