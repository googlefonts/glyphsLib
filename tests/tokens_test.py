...
@pytest.mark.parametrize("test_input,expected,throws", [
    ...
    ("$[name endswith '.sc']", "A.sc", False), # will expand to all glyph names that end in ".sc"
    ...
    # ('$[colorIndex == 5]', "", False),
    # ('$[case == smallCaps]', "", False), # predefined constants: noCase, upper, lower, smallCaps, minor, other
    ('$[name matches "S|s.*"]', "A.sc Sacute", False), # "matches": regular expression
    ...
    ('$[script == "latin"]', "A", False), # connect multiple conditions with ORor AND
    # ('$[hasComponents == true and script == "latin"]', "", False), # connect multiple conditions with ORor AND
    ...
    # ('$[hasSpecialLayers == yes]', "", False), # special layers = color, brace and bracket layers
